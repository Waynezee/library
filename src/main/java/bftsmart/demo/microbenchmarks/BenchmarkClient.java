...
public class BenchmarkClient {
    ...
    public static void main(String[] args) throws IOException {
        if (args.length < 8) {
            // note: number of client is 1
            System.out.println("Usage: ... ThroughputLatencyClient <initial client id> <number of clients> <test time> <payload size> <client rate> <read only?> <verbose?> <nosig | default | ecdsa>");
            System.exit(-1);
        }

        initId = Integer.parseInt(args[0]); 
        int numThreads = Integer.parseInt(args[1]);
        int numberOfOps = Integer.parseInt(args[2]) * 20; // send once every 50ms 
        int payloadSize = Integer.parseInt(args[3]);
        int rate = Integer.parseInt(args[4]);
        int requestSize = (rate/20) * payloadSize;
        int interval = 50;                                // fix to 50ms
        ...
        for (int i = 0; i < numThreads; i++) {
            ...
            clients[i] = new BenchmarkClient.Client(initId + i, numberOfOps, requestSize, interval, readOnly, verbose, s);
        }
            ...
            try {
                ...
                for (int i = 0; i < this.numberOfOps; i++) {
                    ...
                    this.serviceProxy.invokeAsynchRequest(this.request, new ReplyListener() {
                        ...
                        public void replyReceived(RequestContext context, TOMMessage reply) {
                            ...
                            if (replies >= q) {
                                ...
                                System.out.println("Receive: " + context.getReqId() + " " + System.nanoTime());
                    ...
                    if (i > (this.numberOfOps / 2)) {
                        // System.out.println("Receive: " + i + " " + System.nanoTime() + " " + last_send_instant);
                        st.store(System.nanoTime() - last_send_instant);
                    }

                    if (this.interval > 0 || this.rampup > 0) {
                        Thread.sleep(Math.max(this.interval, this.rampup));
